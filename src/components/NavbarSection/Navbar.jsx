...
export default function Navbar({section}) {
    ...
    useEffect(() => {
        if(nameSections[0].displayName !== 'Home Page')
        nameSections.unshift({ displayName: 'Home Page', url: '/' });
    }, []);
    ...
    <>
        <div className="flex justify-between items-center">
            <div className="flex items-center">
                <button  className="sm:mr-3" onClick={handleNavbar}><RxHamburgerMenu/></button>
                
                <button className="mr-3 hidden sm:block" onClick={handleSearch}><IoMdSearch/></button>
                {isVisible && (
            ...
            <CgProfile className='sm:hidden block'/>         
        </div>
        <div className="sm:hidden block h-[1px] bg-gray-600 opacity-25 mt-2 -mx-[12%]"></div>
        <p className='block md:hidden text-right text-[9px] text-blue-950 dark:text-blue-400 mt-2'> SUBSCRIBE FOR 0.50€/WEEK</p>
        {isNavVisible && (
            <div 
                className="h-full fixed top-0 left-0 bg-white shadow-lg z-10 px-4 py-4 "
                onMouseLeave={handleMouseLeave}>
                     <button className='mb-3'>
                        <MdCancel onClick={handleNavbar} fill='black'/>
                    </button>
                    <div className='sm:hidden mb-2 flex'>
                        <input 
                            className='focus:border-neutral-950 border border-gray-500 rounded-sm ml-2 py-1 pl-2' 
                            id='input' 
                            type="text"  
                    ...
                    <a key={index} className='cursor-pointer' href={section.url}>                       
                        <ul>
                            <li key={index} className=' hover:bg-blue-50 px-1 py-1'>
                                <div className='flex items-center justify-between relative' onMouseEnter={() => handleSubDiv(index)} onMouseLeave={handleIndexLeave}>
                                    <a className='font-bold dark:text-black' >{section.displayName}</a>
                                    {section.subsection != null && (<MdKeyboardArrowRight fill='grey'/>)}
                                    
                                    {visibleIndex === index && section.subsection != null && (
                                    <ul className='absolute bg-white z-20 left-full ml-4 border border-black pl-2 pr-12 py-2 w-auto top-0' >
                                        <p className='text-sm text-gray-400 dark:text-black uppercase'>{section.displayName}</p>
                                        {section.subsection.map((sub, index) => (
                                            <li key={index} className='hover:bg-blue-50 whitespace-nowrap'>                              
                                                <a className='font-bold text-sm dark:text-black' href={sub.url}>{sub.name}</a>
                                            </li>
                                        ))}
                                    </ul>
                                )}
